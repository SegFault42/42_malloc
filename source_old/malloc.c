...
{
	if (size <= TINY)
		node->ptr = mmap(NULL, TINY, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
	else if (size <= SMALL)
		node->ptr = mmap(NULL, SMALL, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
	else
		node->ptr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
	if (node->ptr == MAP_FAILED)
	{
		DEBUG_show_ret_mmap((int)node->ptr);
		ft_putstr_fd(RED"MMAP FAILED TO ALLOCATE PTR : "END, 2);
		ft_putendl_fd(strerror(errno), 2);
		return (false);
	}
	if (size <= TINY)
...
void	*create_node()
{
	...
	tmp = mmap(NULL, sizeof(t_block), PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
	if (tmp == MAP_FAILED)
	{
		DEBUG_show_ret_mmap((int)tmp->ptr);
		ft_putstr_fd(RED"MMAP FAILED TO ALLOCATE NEW NODE : "END,2);
		/*ft_putendl_fd(strerror(errno),2);*/
		return (NULL);
	}
...
bool	lst_push_back_large(size_t size)
{
	t_block	*tmp;
...
{
	if (lst_push_back_large(size) == false)
		return (NULL);
	return (init_node(size));
}
